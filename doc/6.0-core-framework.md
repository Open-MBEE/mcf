...
#### DB
The `db.js` file provides an abstraction layer for implementing different
database strategies. In the file are multiple functions and classes which are
required to be defined in the database strategy defined in the running config.
Each strategy must implement the following functions: `connect()`,
`disconnect()`, `clean()` and `sanitize()`. The strategy must also implement 
the classes `Schema`, `Model` and `Store`, all which have a list of required
functions. The Schema and Model classes were modeled closely after 
[Mongoose.js](https://mongoosejs.com/), and the Store class was designed to 
work as a store for [express-session](https://github.com/expressjs/session#session-store-implementation).
Please note: for the server to be started up, **ALL** functions and classes
must be implemented. See the flight manual section `7.0 Database Abstraction`
